...
-o --ofile\toutput grid file\'s name. a .nc extension will be add automaticly\n\
...
def table2nc(infile,ofile,labels,lines,sline,ranges,interval,titles,units,formats,orders):
	...
	fileout.createDimension('x_axis',int((ranges[1]-ranges[0])/interval[0])+1)
	fileout.createDimension('y_axis',int((ranges[3]-ranges[2])/interval[1])+1)
	x = fileout.createVariable('x_axis','d',('x_axis',))
	x.units = labels[0]
	y = fileout.createVariable('y_axis','d',('y_axis',))
	y.units = labels[1]
	...
	for l in range(len(lines)):
		d = fileout.createVariable(titles[l],'d',('y_axis','x_axis'))
		d.units = units[l]
...
def main(argv):
	...
	try:
		opts, args = getopt.getopt(argv,"ho:j:c:r:i:t:u:a:f:d:",["help","ofile=","jumphead=","column=","range=","interval=","title=","unit=","axislabel=","format=","order="])
	...
	table2nc(inputFile,outputFile,axisLabel,dataLine,startLine,dataRange,dataInterval,dataTitle,dataUnit,dataFormat,dataOrder)
